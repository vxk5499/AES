...
#pragma HLS RESOURCE variable=CipherText core=FSL port_map={{CipherText TDATA}}
	...
	forRounds:for(int r=1; r<=10; r++){
		if(r!=10){
			unsigned int e[4], k[4];
			for (int i=0; i<4; i++)
				k[i] = (ExpandedKey[r][0][i] << 24)
				| (ExpandedKey[r][1][i] << 16)
				| (ExpandedKey[r][2][i] << 8)
				| (ExpandedKey[r][3][i]);

			for (int i=0; i<4; i++)
				e[i] = (T0[StateArray[0][i%4]] ^
						T1[StateArray[1][(i+1)%4]] ^
						T2[StateArray[2][(i+2)%4]] ^
						T3[StateArray[3][(i+3)%4]] ^
						k[i]);
